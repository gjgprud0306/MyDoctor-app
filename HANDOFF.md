...
- 2026-08-04 hospital-list-card-comparison-ux-build: 병원 리스트 카드 비교 UX를 보완. 추천순 상태에서 카드 상단에 `추천 이유` 캡션을 노출하고, 평점/리뷰를 노란 정보 그룹으로 강조, 평균 대기/거리에는 시간 아이콘을 추가해 비교 정보를 분리. 가격 인증 배지는 시선을 덜 뺏도록 톤을 낮추고 `총 예상 비용` 라벨/금액 간격을 정리. Figma `970:4245` 노드에도 동일한 카드 정보 그룹/캡션/정렬을 반영하고 스크린샷으로 확인
- 2026-08-04 hospital-list-card-simplify-build: 병원 리스트 카드별 `추천 이유` 캡션을 제거하고 추천 기준 설명은 상단 안내 배너에만 유지. 평점/리뷰의 노란 캡슐 배경을 제거해 `⭐ 평점 · 리뷰` 일반 텍스트로 정리하고, 재진희망률 배지와 총 예상 비용만 파란 강조를 유지. Figma `970:4245`에도 동일하게 반영 후 스크린샷 확인
- 2026-08-04 hospital-list-recommend-notice-single-build: 추천순 선택 시 안내 배너가 2개 연속 노출되던 문제를 수정. 추천순에서는 `총 예상 비용과 재진희망률을 함께 고려한 추천 결과입니다.` 배너만 유지하고, 비용 합산 안내 배너는 추천순이 아닐 때만 노출되도록 조건 처리. 리스트 높이는 배너 1개 기준으로 정리
